<template>
    <div class="frontTop">
        ...
        <nav class="topNav">
            <ul class="firstUl">
                <li class="lif" :class="{curName:navName==item.url}" v-for="(item,index) in navList" @mouseenter.stop="overFirst(item,index)" @mouseleave="outFirst(item,index)" @click.stop="firstGoPage(item,index)">
                    <span>{{item.name}}</span>
                    <i v-if="index!=navList.length-1 && navName!=item.url" class="line">丨</i>
                    <ul class="twoUl" v-if="item.children && navIndex==index">
                        <li class="lit" v-for="(item2,index2) in item.children" @click.stop="twoGoPage(item,item2,index2)">
...
<script>
    export default{
        ...
        data(){
            return {
                navIndex:-1,
                navName:window.sessionStorage.getItem('navName')||'index',
                navList:[
                    {name:'首页', url:'index'},
                    {
                        name:'党建云宣传平台',
        ...
        methods: {
            ...
            outFirst(item,index){
                this.navIndex = -1
                this.navName = window.sessionStorage.getItem('navName')||'index'
            },
            firstGoPage(item,index){
                this.navName = item.url
                this.$router.push({name:item.url})
                window.sessionStorage.setItem('navName',item.url)
            },
            twoGoPage(item,item2,index2){
                this.navName = item.url
                this.$router.push({name:item2.url})
                window.sessionStorage.setItem('navName',item.url)
            }
        }
    }
